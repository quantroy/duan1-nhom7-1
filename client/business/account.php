    ...
    if (isset($_POST['update']) && $_POST['update']) {
        ...
        if (isset($_FILES['image']) && $_FILES['image']['name']) {
            ...
            if ($img_new["size"] > $maxSize) {
                ...
                $sql = "UPDATE accounts SET name = '$name_new', email = '$email_new',updated_at = '$date_upadte', avatar = '$avatar_new', avatar = '$avatar_new' WHERE id = '$id'";
                pdo_execute("$sql");
                $_SESSION['success'] = "Cập nhật thành công";
                header('location:' . BASE_URL . 'trang-chu'  . '?id=' . $id['id']);
            }
        }
        $sql = "UPDATE accounts SET name = '$name_new', email = '$email_new', updated_at = '$date_upadte' WHERE id = '$id'";
        pdo_execute($sql);
        header('location:' . BASE_URL . 'trang-chu'  . '?id=' . $id['id']);
    }
